...
#endif
	bool isParallel;
	bool isParallelEnded;
	int lastWorker;
	int workersCnt;
	bool * workersFinished;
	Worker ** workers;
	long int workersId;
	Tuplesortstate ** worker_states;
};
...
static void computeDatum1(Tuplesortstate *state, SortTuple *stup);
...
void tuplesort_set_prl_level(Tuplesortstate * state, int level) {
	state->workersCnt = level;
}

int  tuplesort_get_prl_level(Tuplesortstate * state) {
	return state->workersCnt;
}

void tuplesort_set_workersId(Tuplesortstate * state, long int id) {
	state->workersId = id;
}

long int tuplesort_get_workersId(Tuplesortstate * state) {
	return state->workersId;
}

void tuplesort_set_parallel(Tuplesortstate * state, bool prl_on) {
	state->isParallel = prl_on;
}

bool tuplesort_is_parallel(Tuplesortstate * state) {
...
void
tuplesort_puttupleslot_prl(Tuplesortstate *state, PrlSortTuple *slot)
{
	MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
	SortTuple	stup;
	 ...
	 * Then call the common code.
	 */
	stup.tuple = (void *)heap_copy_minimal_tuple((MinimalTuple)(slot->tuple));
	USEMEM(state, GetMemoryChunkSpace(stup.tuple));
	// it is a pointer so it will not work just to copy it ... cause it points to shared memory which 
	// will be freed soon
...
bool prl_tuplesort_getsorttuple(Tuplesortstate * state, bool forward, PrlSortTuple * pstup) {
	MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
	SortTuple stup;
	bool should_free;
	if (!tuplesort_gettuple_common(state, forward, &stup, &should_free))
...
static void 
computeDatum1(Tuplesortstate *state, SortTuple * stup) {
	MinimalTuple tuple;
	HeapTupleData htup;
...
void registerWorkers(Tuplesortstate * state, int prl_level) {
	...
	foreach(lc, workersList->list) {
		...
		if (worker->valid && worker->state == PRL_WORKER_STATE_INITIAL && worker->work->jobId == state->workersId) {
...
/**
 * Distribute slot to workers using round robin
 * when last == true then notify workers that all is sent
 */
void distributeToWorker(Tuplesortstate * state, TupleTableSlot * slot,
		bool last) {
	BufferQueueCell * bqc;
	int i, now;
	PrlSortTuple * stup;
...
bool tuplesort_gettupleslot_from_worker(Tuplesortstate * state, bool forward, TupleTableSlot * slot) 
{
	BufferQueueCell * bqc;
	MemoryContext oldcontext;
	SortTuple stup;
	...
	if (state->workersFinished[stup.tupindex] == false) {
		...
		if (bqc->last) {
			...
			// we can check also all workers
			if (state->memtupcount == 0) {
				state->isParallelEnded = true;
			}
		} else {
			oldcontext = MemoryContextSwitchTo(state->sortcontext);
	...
	// free the box which holded the MinimalTuple
	return true;
}

/*
 * Prepare to do the final merge on the fly from workers
 */
void prepareForMerge(Tuplesortstate * state) {
	// prepare heap for tuples
	// in tuplesort_begin_common it was initialized to 1024 .. for now it is enough
	int cw = 0;
	BufferQueueCell * bqc;
	MemoryContext oldcontext;
	SortTuple stup;
	...
	for (cw = 0; cw < state->workersCnt; ++cw) {
		...
		if (bqc->last) {
			...
			oldcontext = MemoryContextSwitchTo(state->sortcontext);
			stup.isnull1 = ((PrlSortTuple *)bqc->ptr_value)->isnull1;
			stup.tuple = (void *)heap_copy_minimal_tuple(((PrlSortTuple *)bqc->ptr_value)->tuple);
			computeDatum1(state, &stup);
			// here exploit this tupindex to store the number of worker it belongs to
			// so we know from where fetch next one
			// it will be set in heap_insert
			tuplesort_heap_insert(state, &stup, cw, false);
			MemoryContextSwitchTo(oldcontext);
	...
	// none of the workers returned anything
	if (state->memtupcount == 0) {
		state->isParallelEnded = true;
	}
...
void prepare_for_multiLTS(Tuplesortstate * state, int cnt) {
	int i = 0;
	state->worker_states = palloc(sizeof(Tuplesortstate *) * cnt);
	state->workersCnt = cnt;
	// set them all as unfinished
	state->workersFinished = (bool *)palloc(sizeof(bool) * state->workersCnt);
	for (i = 0; i < state->workersCnt; ++i) {
		state->workersFinished[i] = false;
...
void register_tuplesort_state(Tuplesortstate * state, Tuplesortstate * worker_state, int position) {
	if (position >= 0 && position < state->workersCnt) {
		state->worker_states[position] = worker_state;
	} else {
		ereport(FATAL,(errmsg("Master - register tuplesort state - wrong position index")));
	}
}
 ...
 */
void preForMergeMultiLTS(Tuplesortstate * state, bool forward) {
	SortTuple stup;
	bool		should_free;
	int clts = 0;
...
bool tuplesort_gettupleslot_from_multiple_lts(Tuplesortstate * state, bool forward, TupleTableSlot * slot) {
	SortTuple stup;
	SortTuple new_stup;
	bool should_free;
	// it is over guys
	...
	tuplesort_heap_siftup(state, false);
	if (state->workersFinished[stup.tupindex] == false) {
		// fetch another one and put it to heap
		if (!tuplesort_gettuple_common(state->worker_states[stup.tupindex], forward, &new_stup, &should_free)) {
			new_stup.tuple = NULL;
			state->workersFinished[stup.tupindex] = true;
			if (state->memtupcount == 0) {
				state->isParallelEnded = true;
...
void copyLtsToSharedMem(Tuplesortstate * state) {
//	state->tapeset
}
